...
# Command argument to OTA command mapping
COMMAND_MAPPING = {"flash": FLASH, "spiffs": SPIFFS, "coredump": COREDUMP}
...
def serve(remote_addr, local_addr, remote_port, local_port, password, filename, command=FLASH):  # noqa: C901
    ...
    message = "%d %d %d %s" % (command, local_port, content_size, file_md5)
    ...
    while inv_tries < 10:
        ...
        try:
            sent = sock2.sendto(message.encode("ascii"), remote_address)  # noqa: F841
        except:  # noqa: E722
    ...
    if data[0] != "OK":
        if data[0] == "AUTH":
            ...
            sys.stderr.flush()
            message = "%d %s %s" % (AUTH, cnonce, result)
            sock2.sendto(message.encode("ascii"), remote_address)
            sock2.settimeout(10)
            try:
                data = sock2.recv(100).decode()
                data = data.split()
            ...
            if data[0] != "OK":
                sys.stderr.write("FAIL\n")
                logging.error("%s", " ".join(data))
                sock2.close()
                return 1

            sys.stderr.write("OK\n")
        else:
            logging.error("Bad Answer: %s", " ".join(data))
            sock2.close()
            return 1
    ...
    try:
        if command == COREDUMP:
            ...
            with open(filename, "wb") as f:
                ...
                while size < transfer_size:
                    ...
                    if not chunk:
                        break
                    size += len(chunk)
                    update_progress(size / float(transfer_size))
                    connection.settimeout(10)
                    try:
                        f.write(chunk)
                        connection.sendall("OK".encode("ascii"))
                    except Exception as e:
                        sys.stderr.write("\n")
                        logging.error("Error Downloading: %s", str(e))
                        return 1

                if size != transfer_size:
                    logging.warning("Incomplete transfer")
                    return 1

                chunk = connection.recv(chunk_size)
                if not chunk:
                    logging.warning("Didn't get hash confirmation")
                    return 1
            ...
            file_md5 = hashlib.md5(open(filename, "rb").read()).hexdigest()
            if data[1] != file_md5:
                logging.warning(f"Transfer MD5 hash invalid, got {data[1]}, expected {file_md5}")
                return 1
...
def parse_args(unparsed_args):
    ...
    parser.add_argument(
        "command", choices=["flash", "spiffs", "coredump"], help="Command to execute: flash, spiffs, or coredump"
    )
